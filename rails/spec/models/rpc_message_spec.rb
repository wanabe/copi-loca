...
RSpec.describe RpcMessage, type: :model do
  ...
  describe "#handle" do
    ...
    it "does nothing if method is not session.event" do
      rpc_message = RpcMessage.create!(session: session, direction: :incoming, message_type: :request, rpc_id: 'id', method: 'other', params: { 'event' => { 'id' => 'e' } })
      expect { rpc_message.handle }.not_to change { Event.count }
    end

    it "does nothing if params['event'] is nil" do
      rpc_message = RpcMessage.create!(session: session, direction: :incoming, message_type: :request, rpc_id: 'id', method: 'session.event', params: {})
      expect { rpc_message.handle }.not_to change { Event.count }
    end

    it "creates event without parent if parentId is nil" do
      rpc_message = RpcMessage.create!(session: session, direction: :incoming, message_type: :request, rpc_id: 'id', method: 'session.event', params: { 'event' => { 'id' => 'e', 'type' => 't', 'data' => {}, 'timestamp' => Time.now, 'ephemeral' => false } })
    ...
    it "creates event with parent if parentId is present" do
      parent = Event.create!(session: session, rpc_message: RpcMessage.create!(session: session, direction: :incoming, message_type: :request, rpc_id: 'pid', method: 'session.event', params: { 'event' => { 'id' => 'pid', 'type' => 't', 'data' => {}, 'timestamp' => Time.now, 'ephemeral' => false } }), event_id: 'pid', event_type: 't', data: {}, timestamp: Time.now)
      rpc_message = RpcMessage.create!(session: session, direction: :incoming, message_type: :request, rpc_id: 'id', method: 'session.event', params: { 'event' => { 'id' => 'e', 'type' => 't', 'data' => {}, 'timestamp' => Time.now, 'ephemeral' => false, 'parentId' => 'pid' } })
      expect { rpc_message.handle }.to change { Event.count }.by(1)
      event = Event.last
      expect(event.event_id).to eq('e')
  ...
  describe ".validations" do
    ...
    it "validates conditional fields for request type" do
      rpc_message = described_class.new(direction: :incoming, message_type: :request, rpc_id: nil, method: "foo", params: { a: 1 }, result: nil, error: nil)
    ...
    it "validates conditional fields for response type" do
      rpc_message = described_class.new(direction: :incoming, message_type: :response, rpc_id: "id", method: nil, params: nil, result: "ok", error: nil)
      allow(rpc_message).to receive(:set_message_type)
      rpc_message.valid?
    ...
    it "validates conditional fields for notification type" do
      rpc_message = described_class.new(direction: :incoming, message_type: :notification, rpc_id: nil, method: "foo", params: { a: 1 }, result: nil, error: nil)
      allow(rpc_message).to receive(:set_message_type)
      rpc_message.valid?
      expect(rpc_message.errors[:rpc_id]).to be_blank
      expect(rpc_message.errors[:method]).to be_blank
