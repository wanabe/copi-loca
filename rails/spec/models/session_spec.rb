...
RSpec.describe Session, type: :model do
  ...
  describe "before_destroy callback" do
    it "deletes associated messages, rpc_messages, events, session_custom_agents, session_tools" do
      ...
      message = Message.create!(session: session, content: 'c', direction: :outgoing, rpc_message_id: rpc_message.id)
      event = Event.create!(session: session, rpc_message: rpc_message, event_id: 'e', event_type: 't', data: {}, timestamp: Time.now)
  ...
  describe "system_message_mode enum" do
    it "has correct values" do
      expect(Session.system_message_modes).to eq({ "default"=>0, "replace"=>1, "append"=>2 })
    end
  end
  ...
  describe "#options" do
    describe "about systemMessage" do
      ...
      it "returns empty options if no branches matched" do
        ...
        allow(session).to receive(:custom_agents).and_return([])
        allow(session).to receive(:tools).and_return([])
        options = session.options
  ...
  describe "#send_prompt" do
    it "creates message if rpc_message found" do
      ...
      allow(session).to receive(:copilot_session).and_return(double(send: 'rpc123'))
      rpc_message = RpcMessage.new(direction: :outgoing, message_type: :request, rpc_id: 'rpc123', method: 'foo', params: {})
      allow(session).to receive(:rpc_messages).and_return(double(find_by: rpc_message))
      allow(session.messages).to receive(:create!).and_return(true)
      session.send_prompt('prompt')
      expect(session.messages).to have_received(:create!).with(content: 'prompt', direction: :outgoing, rpc_message: rpc_message)
    end

    it "raises error and does not create message if rpc_message not found" do
      session = described_class.new(model: 'test')
      allow(session).to receive(:copilot_session).and_return(double(send: 'rpc123'))
      allow(session).to receive(:rpc_messages).and_return(double(find_by: nil))
      allow(session.messages).to receive(:create!)
      expect {
        session.send_prompt('prompt')
      }.to raise_error("RPC message not found for sent prompt with RPC ID: rpc123")
      expect(session.messages).not_to have_received(:create!)
    end

    it "resumes session and reuses it" do
      session = described_class.new(model: 'test')
      copilot_session = double(send: 'rpc123')
      ...
      expect(session.messages).to have_received(:create!).with(content: 'prompt2', direction: :outgoing, rpc_message: rpc_message)
    end
  end
  ...
  describe "#handle" do
    it "creates rpc_message and calls handle" do
      session = described_class.new(model: 'test')
      rpc_message = double(handle: true)
      rpc_messages = double(create!: rpc_message)
      ...
      expect(rpc_messages).to have_received(:create!).with(hash_including(direction: :outgoing, rpc_id: 'rpcid', method: 'foo', params: {}, result: {}, error: nil))
  ...
  describe '#close_after_idle' do
    it 'calls wait_until_idle and then close_session' do
      ...
      expect(called).to eq([ :wait, :close ])
    end
  end
  ...
  describe '#wait_until_idle' do
    it 'invokes the provided block with rpc_message and returns Client.wait result' do
      ...
      copilot_double = double('CopilotSession', idle?: true)
      ...
      rpc_message = double('RpcMessage', session_id: session.id)
    ...
    it 'returns Client.wait result when no block given' do
      copilot_double = double('CopilotSession', idle?: false)
      allow(Client).to receive(:create_session) do |s|
        s.id = SecureRandom.uuid
        copilot_double
      end
  ...
  describe '#close_session' do
    context 'immediately after instance build' do
    ...
    context 'after copilot session is resumed via send_prompt' do
      it 'destroys the created copilot session' do
        copilot_double = double('CopilotSession')
