...
RSpec.describe "GET /sessions/:session_id/rpc_messages", type: :request do
  ...
  let!(:newer_rpc_message) do
    session.rpc_messages.create!(rpc_id: '2', method: 'bar', direction: 'incoming', message_type: 'response', params: nil, result: {}, error: nil)
  end
  ...
  it "filters by method" do
    ...
    expect(css_select(".rpc-messages-flex-body .rpc-col-method").map(&:text)).to contain_exactly('foo')
  end

  it "filters by direction" do
    get "/sessions/#{session.id}/rpc_messages", params: { direction: 'incoming' }
    expect(css_select(".rpc-messages-flex-body .rpc-col-method").map(&:text)).to contain_exactly('bar')
  end
