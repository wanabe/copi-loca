...
describe Copilot::Client do
  ...
  describe ".cache" do
    ...
    it "returns cached client for given url" do
      client1 = described_class.cache(cli_url: cli_url)
      client2 = described_class.cache(cli_url: cli_url)
      expect(client2).to be_a(described_class)
      expect(client2).to equal(client1)
    end

    it "raises error if cli_url is nil" do
      expect { described_class.cache(cli_url: nil) }.to raise_error(/No default CLI URL configured/)
    end
  ...
  describe "#readable?" do
    it "calls wait_readable on socket" do
      ...
      allow(client).to receive(:io).and_return(socket)
      expect(client.readable?(timeout: 1)).to be(true)
    end
  end
  ...
  describe "#pop" do
    ...
    it "parses and stores message" do
      ...
      allow(client).to receive_messages(io: socket, readable?: true)
      allow(socket).to receive_messages(gets: "Content-Length: 10\r\n\r\n", read: '{"id":"id","result":"ok"}')
      allow(logger).to receive(:debug)
    ...
    it "handles method starting with session. or tool. in message" do
      ...
      allow(client).to receive_messages(io: socket, readable?: true)
      allow(socket).to receive_messages(gets: "Content-Length: 10\r\n\r\n",
        read: '{"id":"id","method":"session.create","params":{"sessionId":"sess1"}}')
    ...
    it "handles method not starting with session. or tool. in message" do
      ...
      allow(client).to receive_messages(io: socket, readable?: true)
      allow(socket).to receive_messages(gets: "Content-Length: 10\r\n\r\n",
        read: '{"id":"id","method":"other.method","params":{"sessionId":"sess1"}}')
