...
class Git::EntriesController < ApplicationController
  ...
  before_action :add_git_entries_breadcrumb
  before_action :add_git_entry_breadcrumb, only: [:show]
  before_action :add_git_entry_path_breadcrumb, only: [:show]

  # GET /git/entries
  # @rbs return: void
  def index
    render Views::Git::Entries::Index.new(branches: branches)
  end
  ...
  # @rbs return: void
  def add_git_entries_breadcrumb
    add_breadcrumb("Entries", git_entries_path)
  end

  # @rbs return: void
  def add_git_entry_breadcrumb
    add_breadcrumb parameters.ref, git_entry_root_path(ref: parameters.ref)
  end

  # @rbs return: void
  def add_git_entry_path_breadcrumb
    ...
    parts.each do |part|
      ...
      add_breadcrumb(part, git_entry_path(ref: parameters.ref, path: accumulated_path))
    end
  end
