...
RSpec.describe Git::Grep do
  describe "#run" do
    ...
    it "parses git grep output correctly" do
      git_grep = described_class.new(pattern: pattern, ref: ref, files: files, ignore_case: ignore_case)
      ...
      OUTPUT
      git_grep.run
      expect(Git).to have_received(:call).with("grep", "-i", pattern, ref, "--", "file1.rb", "file2.rb")
    ...
    it "parses git grep output correctly when ref is blank" do
      git_grep = described_class.new(pattern: pattern, ref: nil, files: files, ignore_case: ignore_case)
      allow(Git).to receive(:call).and_return(<<~OUTPUT)
