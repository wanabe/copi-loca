...
describe "POST /git/refs/HEAD/-/stage_line" do
  ...
  it "redirects to new_git_head_path by default" do
    ...
    post stage_line_git_head_path, params: { path: "foo.rb", hunk: hunk, lineno: 4 }
    expect(Git).to have_received(:call).with("apply", "--cached", "--unidiff-zero", "-")
    expect(response).to redirect_to(new_git_head_path(open: "foo.rb"))
  end

  it "redirects to edit_git_head_path if for=edit" do
    allow(Git).to receive(:call).and_yield(StringIO.new).and_return("Applied patch")
    post stage_line_git_head_path, params: { path: "foo.rb", hunk: hunk, lineno: 6, for: "edit" }
    expect(Git).to have_received(:call).with("apply", "--cached", "--unidiff-zero", "-")
